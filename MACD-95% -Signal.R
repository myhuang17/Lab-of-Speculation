...
library(zoo)
library(pkr)
stock = get(getSymbols("TSLA", from="2021-01-01", to=Sys.Date()))
main = function(stocknum, minus){
  #basic information
  #chartSeries(x)
  ...
  bdata = bdata[-((length(bdata[,1]) - minus + 1):length(bdata[,1])),]
  ...
  for(i in 1:2){
    ...
    if(sig[i,2] == 1 & as.numeric(bdata[as.numeric(sig[i,1]),7]) > as.numeric(bdata[as.numeric(sig[(i+1),1]),7])){#MACD為負數
      print("負數MACD沒有上升")
      break
      
    }
    else if(sig[i,2] == 2 & as.numeric(bdata[as.numeric(sig[i,1]),7]) < as.numeric(bdata[as.numeric(sig[(i+1),1]),7])){#MACD為正數
      print("正數MACD沒有下降")
      break
